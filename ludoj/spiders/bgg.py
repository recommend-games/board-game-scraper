...
class BggSpider(Spider):
    ...
    def parse(self, response):
        ...
        for game in response.css('tr#row_'):
            ...
            url = response.urljoin(game.css('td.collection_objectname a::attr(href)')
                                   .extract_first())
            ldr.add_value('url', url)
            image = response.urljoin(game.css('td.collection_thumbnail img::attr(src)')
                                     .extract_first())
            ldr.add_value('image', image)
